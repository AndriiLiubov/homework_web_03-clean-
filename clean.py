...
if __name__ == '__main__':
    ...
    # to run function: python clean.py <folder name>
    ...
    for key, value in registered_extensions.items():
        for file in value:
            if key == "archives":
                th1 = Thread(target=handle_archive, args=(file, folder_path, "archives"))
                th1.start()
                threads.append(th1)
            else:
                th2 = Thread(target=handle_file, args=(file, folder_path, key))
                th2.start()
                threads.append(th2)
